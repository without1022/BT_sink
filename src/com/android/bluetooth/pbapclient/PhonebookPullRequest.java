...
public class PhonebookPullRequest extends PullRequest {
    ...
    public PhonebookPullRequest(Context context, Account account,MainHandler handler) {
		mainHandler = handler;
		Log.e(TAG,"PhonebookPullRequest get the mainHanindle:"+mainHandler);
		Message.obtain(mainHandler,StaticMessage.PROFILE_CONNECT_AVRCP_CONTROLLER,"PhonebookPullRequest get the mainHanindler").sendToTarget();
        mContext = context;
        mAccount = account;
    ...
    public void onPullComplete() {
        ...
        try {
          ...
          /*  contactsToDelete.addAll(contacts.values());
			...
			*/
			for(PhonebookEntry entry:contactsToAdd){
                Log.e(TAG, "onPullComplete: phonebook"+entry.toString() );
            }
